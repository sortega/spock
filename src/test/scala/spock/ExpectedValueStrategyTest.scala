...
class ExpectedValueStrategyTest extends FlatSpec with ShouldMatchers {
  ...
  it should "choose the central elements when having an uniform distribution" in {
    ...
    uniformStrategy.choose(attempt = 2, range = Range(1, 49)) should equal (25 +- 15)
    uniformStrategy.choose(attempt = 3, range = Range(51, 74)) should equal (62 +- 10)
    uniformStrategy.choose(attempt = 1, range = Range(1, 5)) should equal (3 +- 1)
