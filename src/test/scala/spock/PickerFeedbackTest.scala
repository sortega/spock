...
class PickerFeedbackTest extends AnyFlatSpec with Matchers {
  ...
  "Picker feedback" should "be parsed from string" in {
    ...
    Picker.Feedback.parse("guessed 1") shouldBe Some(Guessed(1))
    Picker.Feedback.parse("guessed 5") shouldBe Some(Guessed(5))
    Picker.Feedback.parse("guessed -1") shouldBe None
    Picker.Feedback.parse("guessed 6") shouldBe None
